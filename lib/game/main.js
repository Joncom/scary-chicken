...
MyGame = ig.Game.extend({
    ...
    silent: new ig.Sound('media/audio/silent.*'),
    tracks: [
        new ig.Sound('media/audio/track1.*', false),
        new ig.Sound('media/audio/track2.*', false),
        new ig.Sound('media/audio/track3.*', false),
        new ig.Sound('media/audio/track4.*', false),
        new ig.Sound('media/audio/track5.*', false),
        new ig.Sound('media/audio/track6.*', false),
    ],

    init: function() {
        window.onresize = this.onresize.bind(this);

        // Mobile audio does not work unless triggered by a user
        // input, so hook the user input to enable audio.
        ig.Input.inject({
